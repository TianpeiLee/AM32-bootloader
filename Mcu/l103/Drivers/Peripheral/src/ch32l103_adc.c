 ...
 *
 * @brief   Returns the last ADC1 and conversion result data in dual mode.
 *
...
s32 TempSensor_Volt_To_Temper(s32 Value)
{
    ...
    Refer_Volt = (s32)((TS_Val) & 0x0000FFFF);
    Refer_Temper = (s32)(((TS_Val) >> 16) & 0x0000FFFF);
...
{
    ADCx->CFG &= ~ADC_Sample_Over_1M_Mode;
    ADCx->CFG |= ADC_Sample_Mode;
}

/*********************************************************************
 * @fn      ADC_OffsetCalibrationConfig
 *
 * @brief   Configures the Offset Calibration.
 *
 * @param   ADCx - where x can be 1 to select the ADC peripheral.
 *
 * @return  None
...
void ADC_OffsetCalibrationConfig(ADC_TypeDef *ADCx)
{
    ...
    tmp = ADC_Trim;
    ADCx->CFG &= ~0x0000000F;
    ADCx->CFG |= (uint32_t)((tmp & 0x07) | (((tmp & 0x8000)^0x8000) >> 12));
}

/*********************************************************************
 * @fn      ADC_AnalogWatchdogResetCmd
 *
 * @brief   Enables or disables the analog watch dog reset function.
 *
 * @param   ADCx - where x can be 1 to select the ADC peripheral.
 *          NewState - ENABLE or DISABLE.
 *
